...
// API endpoint
const API_URL = 'http://192.168.0.106:3000/user';
...
const DetailInfoScreen = ({ navigation, route }: DetailInfoScreenProps) => {
    ...
    const uploadCoverPhoto = async (imageUri: string) => {
        ...
        try {
            ...
            // Gửi request
            const response = await fetch('http://192.168.0.106:3000/user/cover/upload', {
                method: 'PUT',
                headers: {
    ...
    const uploadAvatar = async (imageUri: string) => {
        ...
        try {
            ...
            // Gửi request
            const response = await fetch('http://192.168.0.106:3000/user/avatar/upload', {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${accessToken}`,
                    'Content-Type': 'multipart/form-data',
                                    ...
                                    onPress={async () => {
                                        try {
                                            ...
                                            if (
                                                ...
                                                // Make API call
                                                const response = await fetch('http://192.168.0.106:3000/user/profile', {
                                                    method: 'PUT',
                                                    headers: {
                                                        'Authorization': `Bearer ${accessToken}`,
                                                        'Content-Type': 'application/json'
